...
This project and everyone participating in it is governed by the [Code of Conduct](https://github.com/ProfessorManhattan/ansible-ulauncher/blob/master/CODE_OF_CONDUCT.md). By participating, you are expected to uphold this code. Please report unacceptable behavior to [help@megabyte.space](mailto:help@megabyte.space).
...
All pull requests should be associated with issues. Although not strictly required, the pull requests should be made to [the GitLab repository issues board](https://gitlab.com/megabyte-labs/ansible-roles/ulauncher/-/issues) instead of the [GitHub mirror repository](https://github.com/ProfessorManhattan/ansible-ulauncher/issues). This is because we use GitLab as our primary repository and mirror the changes to GitHub for the community.
...
| `docker-ulauncher`           | The same as the `docker` scenario except it excludes platforms that have trouble installing ulauncher packages on Docker.   |
...
| `ci-docker-debian-ulauncher` | Uses Docker to test Debian just like `ci-docker-debian` except it excludes versions that cannot install ulauncher packages. |
...
- macOS is tested using GitHub Actions after the code is automatically synchronized between GitLab and GitHub. ([Link to the macOS GitHub Action configuration](https://github.com/ProfessorManhattan/ansible-ulauncher/-/blob/master/.github/workflows/macOS.yml))
