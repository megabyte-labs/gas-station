...
This project and everyone participating in it is governed by the [Code of Conduct](https://gitlab.com/megabyte-space/ansible-roles/gvisor/-/blob/master/CODE_OF_CONDUCT.md). By participating, you are expected to uphold this code. Please report unacceptable behavior to [help@megabyte.space](mailto:help@megabyte.space).
...
test:gvisor:
  Runs molecule tests using gVisor
test:local:
...
All pull requests should be associated with issues. You can find the [issues board on GitLab](https://gitlab.com/ProfessorManhattan/Playbooks). The pull requests should be made to [the GitLab repository](https://gitlab.com/megabyte-space/ansible-roles/gvisor) instead of the [GitHub repository](https://github.com/ProfessorManhattan/ansible-gvisor). This is because we use GitLab as our primary repository and mirror the changes to GitHub for the community.
...
If there are dependencies that are specific to a certain OS, then the dependency variable should be titled `{{ gvisor }}_dependencies_{{ os_family }}`. For Android Studio, a Fedora-specific dependency list should be named `androidstudio_dependencies_fedora`. In practice, this would look like:
...
Each variable/comment block in `defaults/main.yml` should be seperated by a line return. You can see an example of a `defaults/main.yml` file using this special [variable syntax in the gVisor role](https://gitlab.com/ProfessorManhattan/Playbooks/-/blob/master/roles/virtualization/gvisor/defaults/main.yml).
...
The following is an example of the implementation of action comments. You can find the [source here](https://gitlab.com/ProfessorManhattan/Playbooks/-/blob/master/roles/virtualization/gvisor/tasks/main.yml) as well as an example of why and how you would include an [action comment outside of the `tasks/main.yml` file here](https://gitlab.com/ProfessorManhattan/Playbooks/-/blob/master/roles/virtualization/gvisor/tasks/compose-Darwin.yml).

```yaml
 # @action Ensures gVisor is installed
 # Installs gVisor on the target machine.
 # @action Ensures gVisor is installed
 # Ensures gVisor is started on boot.
 - name: Include tasks based on the operating system
   block:
     - include_tasks: "install-ansible_os_family.yml"
   when: not gvisor_snap_install

 # @action Ensures gVisor is installed
 # If the target gVisor host is a Linux machine and the `gvisor_snap_install` variable
 # is set to true, then gVisor will be installed as a snap package.
 - name: Install gVisor via snap
 ...
 # @action Installs gVisor Compose
 # Installs gVisor Compose if the `gvisor_install_compose` variable is set to true.
 - name: Install gVisor Compose (based on OS)
   block:
     - include_tasks: "compose-ansible_os_family.yml"
   when: gvisor_install_compose | bool
```
...
**Ensures gVisor is installed**

* Installs gVisor on the target machine.
* Ensures gVisor is started on boot.
* If the target gVisor host is a Linux machine and the `gvisor_snap_install` variable is set to true, then gVisor will be installed as a snap package.

**Installs gVisor Compose**
...
Some of our roles include applications like Android Studio. You can not fully test Android Studio from a gVisor command line. In cases like this, you should use our desktop scenarios to provision a desktop GUI-enabled VM to test things like:
