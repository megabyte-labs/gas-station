...
if ($rebootrequired -eq 1) {
    ...
    Write-Host "Rebooting and then run this script again..  " -ForegroundColor Yellow -BackgroundColor DarkGreen
    Write-Host "============================================" -ForegroundColor Yellow -BackgroundColor DarkGreen
    Start-Sleep -s 10
    exit
    Restart-Computer
...
Start-Process "ubuntu2004.exe" -ArgumentList "run curl -sSL https://gitlab.com/megabyte-labs/gas-station/-/raw/master/scripts/quickstart.sh > quickstart.sh" -Wait -NoNewWindow
Start-Process "ubuntu2004.exe" -ArgumentList "run sudo apt-get update && sudo apt-get upgrade -y && bash quickstart.sh" -Wait -NoNewWindow

# @description Output done message
Write-Host "Done." -ForegroundColor Black -BackgroundColor Cyan
