...
function RebootAndContinue {
  ...
  Set-AutoLogon -DefaultUsername "$env:UserName" -DefaultPassword $PASSWORD -Script "c:\Temp\quickstart.ps1"
  Restart-Computer
}
...
function EnableWinRM {
  ...
  $USERNAME = $env:USERNAME.ToLower()
  $OPENSSL_CONF = "C:\Temp\openssl.conf"
...
"@
  ...
  $user_path = Join-Path "C:\Temp" user.pem
  $key_path = Join-Path "C:\Temp" key.pem

  $env:OPENSSL_CONF=$OPENSSL_CONF
  & "C:\Program Files\OpenSSL-Win64\bin\openssl.exe" req -x509 -nodes -days 365 -newkey rsa:2048 -out $user_path -outform PEM -keyout $key_path -subj "/CN=$USERNAME" -extensions v3_req_client 2>&1

  $env:OPENSSL_CONF=""
  Remove-Item $OPENSSL_CONF -Force

  Import-Certificate -FilePath $user_path -CertStoreLocation cert:\LocalMachine\root
  $cert=Import-Certificate -FilePath $user_path -CertStoreLocation cert:\LocalMachine\TrustedPeople

  $PasswordCred = ConvertTo-SecureString -AsPlainText -Force $PASSWORD
  $creds = New-Object System.Management.Automation.PSCredential($USERNAME, $PasswordCred) -ea Stop

  New-Item -Path WSMan:\localhost\ClientCertificate -Subject "$USERNAME@localhost" -URI * `
    -Issuer $cert.Thumbprint -Credential $creds -Force
...
function RunPlaybook {
  ...
  Invoke-WebRequest -Uri https://gitlab.com/megabyte-labs/gas-station/-/raw/master/scripts/quickstart.sh -OutFile $("$($CurrentLocation)"+"\quickstart.sh")
